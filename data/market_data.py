...
class MarketDataEngine:
    ...
    def set_connection(self, connection):
        ...
        # Tarea para listar símbolos disponibles
        asyncio.create_task(self._log_symbols())

    async def _log_symbols(self):
        """Log de todos los símbolos disponibles en la cuenta."""
        try:
            await asyncio.sleep(5)
            symbols = await self._connection.get_symbols()
            logger.info(f"SIMBOLOS DISPONIBLES ({len(symbols)}):")
            for s in symbols:
                if any(k in s.upper() for k in ["BTC", "ETH", "XAU", "EUR", "NAS", "US30", "US100"]):
                    logger.info(f"  → {s}")
