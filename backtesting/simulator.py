...
class Simulator:
    ...
    def simulate_trades(
        ...
        for i in range(lookback + 5, len(bars) - 10):
            ...
            # Los nuevos módulos actúan como multiplicadores
            # sobre señales BOS/OB/FVG — no como entradas propias.
            # Un trade puede ser potenciado o filtrado.
            # ═══════════════════════════════════════════════════
            quality_mult = 1.0

            # ── Displacement: confirma o niega el impulso ─────
            if i >= 10:
                d_score, d_valid = displacement_score(bars, i, atr_val)
                if d_valid:
                    d_dir = "buy" if bar.close > bar.open else "sell"
                    if d_dir == direction:
                        quality_mult *= 1.18   # impulso alineado → boost
                        pattern = pattern + "_DISP"
                    else:
                        quality_mult *= 0.82   # impulso contrario → penalizar
                elif d_score > 0.3:
                    quality_mult *= 1.05       # impulso parcial → leve boost

            # ── Judas Swing: detecta trampa de sesión ─────────
            if i >= 20:
                js = analyze_judas_swing(bars, i, atr_val)
                if js.detected:
                    if direction == js.real_direction:
                        quality_mult *= 1.25   # operando con el movimiento real
                        pattern = pattern + "_JS"
                    elif direction == js.judas_direction:
                        quality_mult *= 0.55   # operando con la trampa → penalizar fuerte

            # ── Inducción: valida o invalida por EQH/EQL ─────
            if i >= 30:
                ind = analyze_induccion(bars, i, atr_val)
                if ind.detected:
                    if direction == ind.real_direction:
                        quality_mult *= 1.20   # operando con la trampa real
                        pattern = pattern + "_IND"
                    else:
                        quality_mult *= 0.65   # yendo contra la trampa real

            # Aplicar multiplicador al score
            score = min(1.0, score * quality_mult)

            if score < score_threshold:
                continue  # filtrado por módulos de calidad
