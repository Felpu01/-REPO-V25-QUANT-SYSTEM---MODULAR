"""
BACKTESTING ENGINE INSTITUCIONAL — V3
Fuente primaria: MetaAPI → conexión RPC activa (get_historical_candles)
Fallback: Binance Vision (BTC/ETH) + Dukascopy (EUR/XAU/NAS)
DuckDB resampling | Walk-Forward 70/30 | Monte Carlo | Checkpoints
"""
...
# Fallback para nombres genéricos
BINANCE_SYMBOLS = {
    "BTCUSDm": "BTCUSDT",
    "ETHUSDm": "ETHUSDT",
}
...
# Spread real observado de Exness (bid/ask del log)
SPREAD_REAL = {
    "BTCUSDm": 14.0,
    "ETHUSDm": 1.40,
    "XAUUSDm": 0.28,
...
# Timeframe MetaAPI SDK: mismo formato que market_data.py
TF_META = {"M1": "1m", "M5": "5m", "M15": "15m", "H1": "1h", "H4": "4h", "D1": "1d"}
...
# ─── MetaAPI Fetcher — usa conexión RPC activa ────────────────

class MetaAPIFetcher:
    """
    FIX: Usa la conexión RPC activa (get_historical_candles via SDK websocket)
    en lugar del endpoint REST de pago (/historical-market-data/).
    La misma conexión que ya usa market_data.py para datos en vivo.
    """
    ...
    async def fetch_symbol(
        ...
        """Descarga toda la historia disponible para un símbolo via RPC."""
        cache_file = os.path.join(CACHE_DIR, f"{symbol}_{timeframe}_exness.json")

        if os.path.exists(cache_file):
            logger.info(f"MetaAPI cache hit: {symbol} {timeframe}")
            with open(cache_file, "r") as f:
                data = json.load(f)
            return [Bar(**b) for b in data]

        if not self._connection:
            logger.warning(f"MetaAPI: sin conexión RPC para {symbol}")
            return []

        logger.info(f"Descargando {symbol} desde MetaAPI/Exness RPC ({start_year}-presente)...")
        all_bars = []
        spread   = SPREAD_REAL.get(symbol, 0)
        now      = datetime.now(timezone.utc)
        minutes  = TF_MINUTES.get(timeframe, 60)
        # Pedir de a 1000 barras por chunk
        chunk_size = 1000

        # Arrancar desde start_year e ir avanzando
        ...
        while current_start < now:
            try:
                ...
                if not candles:
                    # Sin datos en este rango — avanzar al siguiente período
                ...
                chunk_bars = [b for b in chunk_bars if b.close > 0]
                if chunk_bars:
                ...
                logger.debug(f"MetaAPI RPC {symbol} chunk: {e}")
        ...
        if all_bars:
            ...
            logger.info(
                f"✅ MetaAPI {symbol}: {len(all_bars)} barras {timeframe} cacheadas "
                f"({all_bars[0].time[:10]} → {all_bars[-1].time[:10]})"
            )
        else:
            logger.warning(f"MetaAPI {symbol}: sin datos via RPC")

        return all_bars

    async def close(self):
        pass  # La conexión la maneja main.py
...
class DukascopyFetcher:
    ...
    async def fetch_symbol(self, symbol: str, start_year: int = 2003) -> list:
        ...
        for year in range(start_year, now.year + 1):
            ...
            logger.info(f"  {symbol} {year}: {len([b for b in all_bars if b.time[:4] == str(year)])} H1")
    ...
    def _parse_bi5(self, data, symbol, year, month):
        try:
            ...
            bars = []
            spread = SPREAD_REAL.get(symbol, 0)
            price_factor = {"EURUSDm": 100000.0, "XAUUSDm": 1000.0, "USTECm": 10.0}.get(symbol, 1.0)
...
class BacktestEngine:
    def __init__(self, token: str = "", account_id: str = "", connection=None):
        ...
        self._connection = connection  # FIX: conexión RPC activa desde main.py

        # FIX: MetaAPIFetcher ahora recibe la conexión RPC, no token/account_id
        self.metaapi   = MetaAPIFetcher(connection) if connection else None
        ...
        source = "MetaAPI RPC/Exness + Binance + Dukascopy"
        logger.info(f"BacktestEngine V3 iniciado — {source}")
        if connection:
            logger.info("✅ Conexión RPC activa — datos históricos reales de Exness")
        else:
            logger.warning("⚠️  Sin conexión RPC — usando solo Binance/Dukascopy")
    ...
    async def run_all(self, symbols: list = None) -> dict:
        ...
        results = {}
        # Secuencial para no saturar la conexión RPC
        for sym in symbols:
            try:
    ...
    async def _fetch_data(self, symbol: str, timeframe: str) -> tuple:
        ...
        # ── 1. MetaAPI RPC (fuente primaria — conexión activa) ─
        if self.metaapi and self._connection:
            logger.info(f"{symbol}: intentando MetaAPI RPC/Exness...")
            bars = await self.metaapi.fetch_symbol(symbol, timeframe, start_year)
            if len(bars) >= 200:
                return bars, f"MetaAPI/Exness {timeframe}"
            else:
                logger.warning(f"{symbol}: MetaAPI RPC insuficiente ({len(bars)} barras), usando fallback")
    ...
    async def close(self):
        ...
        # MetaAPIFetcher no cierra la conexión — la maneja main.py
