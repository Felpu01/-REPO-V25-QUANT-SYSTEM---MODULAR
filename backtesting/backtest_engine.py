...
Fuente primaria: MetaAPI → MT5/Exness (datos reales bid/ask/spread)
...
TF_METAAPI = {"M1": "1m", "M5": "5m", "M15": "15m", "H1": "1h", "H4": "4h", "D1": "1d"}
...
# ─── MetaAPI Fetcher (Fuente Primaria) ────────────────────────

class MetaAPIFetcher:
    """
    Descarga datos históricos reales desde MT5/Exness via MetaAPI REST.
    Usa el mismo token que el bot en vivo — datos con spread real.
    """
    ...
    async def fetch_symbol(
        ...
        logger.info(f"Descargando {symbol} desde MetaAPI/Exness ({start_year}-presente)...")
        all_bars = []
        tf_str   = TF_METAAPI.get(timeframe, "1h")
        spread   = SPREAD_REAL.get(symbol, 0)
        now      = datetime.now(timezone.utc)

        # Descargar año por año
        for year in range(start_year, now.year + 1):
            start_time = datetime(year, 1, 1, tzinfo=timezone.utc)
            end_time   = datetime(min(year + 1, now.year + 1), 1, 1, tzinfo=timezone.utc)
            if end_time > now:
                end_time = now

            year_bars = await self._fetch_range(
                symbol, tf_str, start_time, end_time, spread
            )

            if year_bars:
                all_bars.extend(year_bars)
                logger.info(f"  {symbol} {year}: {len(year_bars)} barras {timeframe}")
            else:
                logger.debug(f"  {symbol} {year}: sin datos")
        ...
        if all_bars:
            ...
            logger.warning(f"MetaAPI {symbol}: sin datos disponibles")

        return all_bars

    async def _fetch_range(
        self, symbol: str, tf: str,
        start: datetime, end: datetime, spread: float
    ) -> list[Bar]:
        """Fetch barras para un rango de tiempo específico."""
        all_bars = []
        current_start = start
        limit = 1000  # máximo por request

        while current_start < end:
            bars = await self._fetch_chunk(symbol, tf, current_start, limit, spread)
            if not bars:
                break
        ...
        return [b for b in all_bars if b.time[:10] >= start.isoformat()[:10]]

    async def _fetch_chunk(
        self, symbol: str, tf: str,
        start_time: datetime, limit: int, spread: float
    ) -> list[Bar]:
        """Fetch un chunk de barras desde MetaAPI."""
        try:
            sess = await self._get_session()
            start_str = start_time.strftime("%Y-%m-%dT%H:%M:%S.000Z")
            url = (
                f"{METAAPI_BASE}/users/current/accounts/{self._account_id}"
                f"/historical-market-data/symbols/{symbol}"
                f"/timeframes/{tf}/candles"
                f"?limit={limit}&startTime={start_str}"
            )
            async with sess.get(url, timeout=aiohttp.ClientTimeout(total=30)) as r:
                if r.status != 200:
                    logger.debug(f"MetaAPI {symbol} {tf}: HTTP {r.status}")
                    return []
                data = await r.json()
...
class BacktestEngine:
    def __init__(self, token: str = "", account_id: str = ""):
        ...
        source = "MetaAPI/Exness + Binance + Dukascopy"
    ...
    async def _fetch_data(self, symbol: str, timeframe: str) -> tuple:
        ...
        # ── 1. MetaAPI/Exness (fuente primaria) ───────────────
        if self.metaapi and self._token:
            logger.info(f"{symbol}: intentando MetaAPI/Exness...")
            bars = await self.metaapi.fetch_symbol(symbol, timeframe, start_year)
            if len(bars) >= 200:
                return bars, f"MetaAPI/Exness {timeframe}"
            else:
                logger.warning(f"{symbol}: MetaAPI insuficiente ({len(bars)} barras), usando fallback")
