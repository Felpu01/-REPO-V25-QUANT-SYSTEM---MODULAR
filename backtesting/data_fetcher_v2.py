...
Fixes v2:
  ...
  - cache_dir configurable → usa Railway Volume (/data) si está montado
"""
...
# ─── Cache dir: Railway Volume si existe, local si no ─────────
# Con Railway Volume montado en /data → persiste entre redeploys
# Sin Volume → usa ./backtest_cache (ephemeral, no crashea)
        ...
        for attempt in range(MAX_RETRIES):
            try:
                ...
                logger.warning(
                    ...
                    f"timeout (intento {attempt+1}/{MAX_RETRIES}) — recreando sesión"
                )
                if self._session and not self._session.closed:
                    await self._session.close()
                self._session   = None
...
class DataFetcher:
    ...
    cache_dir: directorio donde guardar datos.
      - Si se pasa explícitamente (desde backtest_engine) usa ese.
      - Si no, detecta automáticamente: /data si Railway Volume
        está montado, ./backtest_cache si no.
    """
