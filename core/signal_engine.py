def generate_signal(
    regime,
    score,
    bias,
    bias_memory,
    bos_up,
    bos_down,
    choch_up,
    choch_down,
    sweep_up,
    sweep_down,
    displacement_valid,
    volatility
):

    # =========================
    # SCORE ZONES (AJUSTADAS V35.3)
    # =========================
    strong = score >= 0.65
    mid = score >= 0.50
    weak = score >= 0.40

    bullish_bias = bias == "BULLISH"
    bearish_bias = bias == "BEARISH"

    structure_bull = bos_up or choch_up
    structure_bear = bos_down or choch_down

    liquidity_bull = sweep_down
    liquidity_bear = sweep_up

    market_ok = regime in ["TREND", "EXPANSION"]

    momentum_ok = volatility > 0.40  # 🔥 más permisivo (clave del fix)

    # =========================
    # FILTRO BASE
    # =========================
    if not market_ok:
        return "WAIT"

    if not momentum_ok:
        return "WAIT"

    # =========================
    # BUY LOGIC
    # =========================

    # 🔥 entrada fuerte institucional
    if strong and bullish_bias:
        if structure_bull or liquidity_bull or displacement_valid:
            return "BUY"

    # 🔥 entrada media (más activa)
    if mid and bullish_bias:
        if structure_bull or liquidity_bull or displacement_valid:
            return "BUY"

    # 🔥 entrada débil pero con contexto
    if weak and bullish_bias and bias_memory > 0.5:
        if structure_bull or displacement_valid:
            return "BUY"

    # =========================
    # SELL LOGIC
    # =========================

    # 🔥 entrada fuerte institucional
    if strong and bearish_bias:
        if structure_bear or liquidity_bear or displacement_valid:
            return "SELL"

    # 🔥 entrada media
    if mid and bearish_bias:
        if structure_bear or liquidity_bear or displacement_valid:
            return "SELL"

    # 🔥 entrada débil con contexto
    if weak and bearish_bias and bias_memory < -0.5:
        if structure_bear or displacement_valid:
            return "SELL"

    return "WAIT"
