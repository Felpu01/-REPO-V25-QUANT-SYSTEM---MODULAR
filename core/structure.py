...
class SMCEngine:
    ...
    def _structure_bias(self, swings) -> str:
        if len(swings) < 4:
            return "neutral"
        last = swings[-6:]
    ...
    def _detect_bos(self, bars, swings, bias) -> tuple:
        if not swings or len(bars) < BOS_LOOKBACK:
            return False, 0.0
        recent = bars[-BOS_LOOKBACK:]
        if bias == "bullish":
            sh = next((s for s in reversed(swings) if s.type in ["LH", "HH"]), None)
            if sh:
                closes_above = [b for b in recent[-5:] if b.close > sh.price]
                if len(closes_above) >= 1:
                    return True, sh.price
        elif bias == "bearish":
            sl = next((s for s in reversed(swings) if s.type in ["HL", "LL"]), None)
            if sl:
                closes_below = [b for b in recent[-5:] if b.close < sl.price]
                if len(closes_below) >= 1:
                    return True, sl.price
        return False, 0.0

    def _detect_choch(self, bars, swings, bias) -> tuple:
        if len(swings) < 4:
            return False, 0.0
        last_4 = swings[-4:]
        if bias == "bearish" and last_4[-1].type == "HL":
            return True, last_4[-1].price
        elif bias == "bullish" and last_4[-1].type == "LH":
            return True, last_4[-1].price
        return False, 0.0

    def _detect_ob(self, bars, bias) -> list:
        obs = []
    ...
    def _detect_fvg(self, bars) -> list:
        fvgs = []
        for i in range(1, len(bars) - 1):
            ...
            if nxt.low > prev.high:
                ...
                if size / (price_ref + 1e-9) >= FVG_MIN_PCT:
                    fvg = FVG(index=i, type="bullish",
    ...
    def _detect_liquidity(self, bars) -> list:
        levels = []
        recent = bars[-80:]
        current = bars[-1].close
    ...
    def _premium_discount(self, bars) -> tuple:
        recent = bars[-50:]
        ...
        pos = (current - sl) / rng
        if pos > 0.62:   return "premium", eq
        elif pos < 0.38: return "discount", eq
        return "equilibrium", eq
...
class MultiTFSMC:
    ...
    def analyze_all(self, mtf: MultiTF) -> dict:
        results = {}
    ...
    def get_htf_bias(self, analyses: dict) -> str:
        biases = []
        for tf in ["D1", "H4"]:
            if tf in analyses:
    ...
    def get_entry_tf_score(self, analyses: dict, direction: str) -> float:
