"""
QUANT SCORING ENGINE — Fase 2 + Macro
Score probabilístico multi-timeframe institucional.
Integra: SMC + Sweep + FVG + News + Learning + Macro + Manipulation + Probability.
"""
...
class QuantScoringEngine:
    ...
    async def analyze(self, mtf: MultiTF) -> QuantResult:
        ...
        try:
            ...
            macro_sym_score = macro.get_symbol_score(symbol)
            macro_bias = macro.macro_bias.get(symbol, "neutral")
            result.macro_score = macro_sym_score
            result.macro_bias = macro_bias
            result.risk_sentiment = macro.risk_sentiment
            if macro.warnings:
                for w in macro.warnings:
                    logger.info(f"MACRO WARNING {symbol}: {w}")
        ...
        # Validar alineación macro
        if result.macro_bias != "neutral" and result.macro_bias != direction:
            result.macro_score *= 0.5
            logger.debug(f"{symbol} macro bias {result.macro_bias} vs direction {direction}")
        ...
        if h1_data:
            ...
            if sweep_analysis.recent_sweep:
                if sweep_analysis.recent_sweep.direction != direction:
        ...
        if h1_data:
            ...
            # Bonus si manipulación alinea con dirección
            if manip.most_recent and manip.most_recent.direction == direction:
                result.manipulation_score = min(1.0, result.manipulation_score * 1.2)
            elif manip.most_recent and manip.most_recent.direction != direction:
        ...
        # ── 13. Score Final Institucional ─────────────────────
        result.final_score = (
            result.smc_score          * 0.20 +
            result.entry_tf_score     * 0.12 +
            result.sweep_score        * 0.12 +
            result.probability_score  * 0.12 +
            result.fvg_score          * 0.08 +
            result.manipulation_score * 0.08 +
            result.momentum_score     * 0.10 +
            result.confluence_score   * 0.08 +
            result.macro_score        * 0.05 +
    ...
    def _generate_setup(self, result, analyses: dict, mtf: MultiTF):
        ...
        if ob is None and mtf.H1:
            ...
            premium = [
                f for f in fvgs
                if f.is_premium and f.type == ("bullish" if direction == "buy" else "bearish")
            ]
            if premium:
                ob_fvg = premium[0]
                class FVGasOB:
