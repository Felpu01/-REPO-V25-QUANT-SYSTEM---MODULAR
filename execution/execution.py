...
class RiskManager:
    ...
    def calculate_lot_size(
        ...
        # Lot size institucional simplificado
        # Para cripto: 1 lot = 1 unidad | Forex: 1 lot = 100000 | Índices: variable
...
class MT5Executor:
    def __init__(self):
        self._token   = META_API_TOKEN or os.getenv("META_API_TOKEN", "")
        self._acct_id = MT5_ACCOUNT_ID or os.getenv("MT5_ACCOUNT_ID", "")
        self._session: Optional[aiohttp.ClientSession] = None
        self._positions: dict[str, Position] = {}
        self._risk    = RiskManager()
        self._connected = False
        self._balance   = BALANCE_START
        logger.info("MT5Executor iniciado — MetaAPI mode")
    ...
    async def send_order(
        ...
        # Recalcular lots con risk manager
        lots = self._risk.calculate_lot_size(symbol, entry, sl)
        if lots <= 0:
            return TradeResult(success=False, error="Lot size inválido")
        ...
        try:
            ...
            async with sess.post(
                ...
                if r.status in [200, 201] and data.get("orderId"):
                    ...
                    logger.info(
                        f"✅ ORDER SENT | {symbol} {direction.upper()} "
                        f"{lots} lots @ {entry} | SL:{sl} TP:{tp} | ID:{order_id}"
                    )
    ...
    async def _sim_order(
        ...
        """Simula orden cuando MetaAPI no está configurado."""
        import random
    ...
    async def modify_sl(self, order_id: str, new_sl: float) -> bool:
        """Modifica SL — trailing stop / break even."""
    ...
    async def close_position(self, order_id: str, reason: str = "") -> bool:
        ...
        if not self._connected:
            logger.info(f"SIM CLOSE | {pos.symbol} {pos.direction} | {reason}")
    ...
    async def manage_positions(self, current_prices: dict[str, float]):
        """Trailing stop + break even para posiciones abiertas."""
        for order_id, pos in list(self._positions.items()):
            price = current_prices.get(pos.symbol, 0)
            if price == 0:
                continue

            cfg = SYMBOL_CONFIG.get(pos.symbol, {})
            atr_approx = (pos.entry - pos.sl) * 1.5  # estimación

            if pos.direction == "buy":
                profit_pips = price - pos.entry

                # Break even: si ganó 1 ATR, mover SL a entry
                if not pos.be_activated and profit_pips >= atr_approx:
                ...
                # Trailing stop: si ganó 2 ATR
                elif pos.be_activated and profit_pips >= atr_approx * 2:
                    trail_sl = price - atr_approx * 0.8
                    if trail_sl > pos.sl:
                        await self.modify_sl(order_id, round(trail_sl, 5))

            elif pos.direction == "sell":
                profit_pips = pos.entry - price
