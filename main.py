...
for i in range(20, len(prices)):
    ...
    # 🔥 FIX CRÍTICO: NORMALIZAR TREND
    if isinstance(tr_raw, str):
        tr_dir = 1 if tr_raw.upper() in ["UP", "BULL", "BULLISH", "BUY"] else -1
    else:
        tr_dir = 1 if tr_raw > 0 else -1
    ...
    # =========================
    alignment_ok = (
        (bias == "BULLISH" and tr_dir > 0) or
        (bias == "BEARISH" and tr_dir < 0)
    )
