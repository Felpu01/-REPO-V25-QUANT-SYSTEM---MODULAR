...
prices = [x.get("price", x) for x in data] if isinstance(first, dict) else data
...
for i in range(20, len(prices)):
    ...
    if result:
        ...
        close_reason = result.get("close_reason", None)
    ...
    # RETEST ENGINE
    # =====================
    bullish_retest, bearish_retest = retest_entry(price, prev_high, prev_low, bias)
    ...
    if cooldown_manager.allowed_to_trade():
        ...
        if strong and bullish and good_market and displacement_valid:
            signal = "BUY"
