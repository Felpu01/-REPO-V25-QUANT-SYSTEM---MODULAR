...
for i in range(20, len(prices)):
    ...
    if result is not None:
        ...
        trade_result = result.get("close_reason", "NONE")
        ...
        cooldown_manager.register_trade(trade_result)
    ...
    if regime == "EXPANSION":
        ...
        sc -= 0.05   # 🔥 FIX: menos penalización (ANTES era -0.10)

    sc = max(0.0, min(sc, 1.0))

    # =====================
    # SIGNAL ENGINE (FIXED V34.3)
    # =====================
    signal = "WAIT"

    allowed_trade = cooldown_manager.allowed_to_trade()
    ...
    if not allowed_trade:
        ...
        # 🔥 CORE LOGIC SIMPLIFIED (menos rigidez, más ejecución real)
        strong_signal = sc >= 0.85
        valid_context = structure_event and displacement_valid

        bullish_ok = bias == "BULLISH" and bias_memory > 0.5
        bearish_ok = bias == "BEARISH" and bias_memory < -0.5

        buy_confirmed = strong_signal and bullish_ok and bullish_retest
        sell_confirmed = strong_signal and bearish_ok and bearish_retest

        # 🔥 STRUCTURE OVERRIDE (IMPORTANTE)
        if valid_context and sc >= 0.90:
            buy_confirmed = buy_confirmed or (bias == "BULLISH")
            sell_confirmed = sell_confirmed or (bias == "BEARISH")
