...
for i in range(10, len(prices)):
    ...
    if bos_up:
        bias_memory += 1.0
    if choch_up:
        bias_memory += 1.0   # FIX: más peso institucional
    if sweep_down:
        bias_memory += 0.6
    ...
    # =========================
    # STRUCTURE LOCK (INSTITUTIONAL CORE FIX)
    # =========================
    if choch_up:
        bias = "BULLISH"
        ...
        if bias_memory > 0.7:
            ...
            bias = "BEARISH"
        # else KEEP LAST BIAS (no flip noise)
    ...
    # =========================
    # STRUCTURE FILTER (REDUCED OVERFITTING)
    # =========================
    ...
    # =========================
    # LIQUIDITY BOOST (REALISTIC BOOST)
    # =========================
    if sweep_up or sweep_down:
        sc += 0.25   # FIX: más impacto (antes 0.20)

    if choch_up or choch_down:
        sc += 0.15   # FIX: más institucional

    if bos_up or bos_down:
        sc += 0.08   # FIX: leve ajuste
    ...
    # =========================
    # SIGNAL LOGIC (FIXED ACTIVITY LEVEL)
    # =========================
    signal = "WAIT"
