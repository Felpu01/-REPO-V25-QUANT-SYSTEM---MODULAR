...
for i in range(20, len(prices)):
    ...
    # =========================
    # STRUCTURE ACTIVITY SCORE (FIX CORE)
    # =========================
    structure_score = 0
    ...
    structure_event = structure_score >= 0.8   # 🔥 FIX: menos estricto

    # =========================
    # BIAS MEMORY (BALANCED)
    # =========================
    bias_memory *= 0.88
    ...
    # =========================
    # FILTERS (LESS AGGRESSIVE FIX)
    # =========================
    if not structure_event:
        sc *= 0.75   # 🔥 FIX: antes 0.55 (muy agresivo)
    ...
    if regime == "EXPANSION":
        ...
        sc -= 0.03   # menos castigo

    sc = min(sc, 1.0)

    # =========================
    # ENTRY CONDITIONS (FIX BALANCE)
    # =========================
    alignment = (
        (bias == "BULLISH" and (bos_up or choch_up or sweep_down)) or
        (bias == "BEARISH" and (bos_down or choch_down or sweep_up))
    )

    quality = structure_event and abs(bias_memory) > 1.0
