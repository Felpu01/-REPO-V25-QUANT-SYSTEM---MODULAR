...
for i in range(10, len(data)):
    ...
    # reset si pasa demasiado tiempo sin confirmación
    ...
    # =========================
    # SIGNAL LOGIC (SMC + MEMORY FIX)
    # =========================
    signal = "WAIT"

    if sc >= config.SCORE_THRESHOLD:

        # BUY (sweep abajo + confirmación estructural)
        if active_sweep == "BUY" and (bos_up or choch_up):
            signal = "BUY"
            active_sweep = None
            sweep_index = None

        # SELL (sweep arriba + confirmación estructural)
        elif active_sweep == "SELL" and (bos_down or choch_down):
            signal = "SELL"
            active_sweep = None
            sweep_index = None
