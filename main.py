...
print("🚀 V36 QUANT SYSTEM STARTED (INSTITUTIONAL STABLE)")
...
for i in range(20, len(prices)):
    ...
    # 🔥 más realista: evita sobre-trading en RANGE falso
    ...
    # 🔥 anti-overreaction fix
    bias_memory *= 0.97

    bias = (
        "BULLISH" if bias_memory > 0.55
        else "BEARISH" if bias_memory < -0.55
        else "NEUTRAL"
    )
    ...
    # =====================
    # ENTRY QUALITY FILTER (🔥 TU MEJORA CLAVE)
    # =====================
    entry_quality = (
        (bos_up or bos_down or choch_up or choch_down) and
        (displacement_valid or sweep_up or sweep_down) and
        vol > 0.40
    )


    # =====================
    # COOLDOWN + EXECUTION CONTROL (V36 FIX)
    # =====================
    force_trade = (
        sc >= 0.88 and
        bias != "NEUTRAL" and
