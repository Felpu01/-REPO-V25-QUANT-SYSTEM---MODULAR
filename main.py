...
if runtime:
    ...
    breakevens = runtime.get(
        "breakevens",
        0
    )

    total_trades = runtime.get(
        "total_trades",
        0
    )

    bias_memory = runtime.get(
        "bias_memory",
        0.0
    )

    saved_cooldown = runtime.get(
        "cooldown",
        0
    )
...
for i in range(20, len(prices)):
    ...
    if result is not None:
        ...
        elif result["result"] == "BE":

            breakevens += 1

        cooldown_manager.register_trade(
            result["result"]
        )
