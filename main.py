...
for i in range(10, len(prices)):
    ...
    # =========================
    # 🚨 EXHAUSTION FILTER (NEW FIX)
    # =========================
    impulse = abs(price - prices[i-5])
    avg_move = sum(abs(prices[j] - prices[j-1]) for j in range(i-5, i)) / 5

    late_expansion = impulse > avg_move * 3.0
    ...
    if sc >= config.SCORE_THRESHOLD and valid_context and not late_expansion:
