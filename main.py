...
for i in range(20, len(prices)):
    ...
    if vol > 0.78 and (bos_up or bos_down):
        regime = "EXPANSION"
    elif range_size < 28 and vol < 0.60:
        regime = "RANGE"
    else:
        regime = "TREND"
    ...
    if sweep_down:
        bias_memory += 0.6
    if displacement_valid and displacement_strength < 2.0:
        bias_memory += 0.4   # 🔥 LIMITAMOS EXCESO
    ...
    # 🔥 HARD FILTER (CLAVE V39)
    if displacement_strength > 2.2:
        sc *= 0.7   # evita spikes falsos
    ...
    if cooldown_manager.get_cooldown() > 0 and not high_quality_entry:
        signal = "WAIT"
    else:
        signal = base_signal
