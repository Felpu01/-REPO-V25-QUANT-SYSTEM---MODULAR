...
for i in range(20, len(prices)):
    ...
    if not allowed_trade:
        ...
        bias_strength = bias_memory

        bullish = bias_strength > 0.25
        bearish = bias_strength < -0.25

        structure_ok = structure_event or bos_up or bos_down or choch_up or choch_down
        good_market = regime in ["TREND", "EXPANSION"]

        # 🟢 STRONG
        buy_strong = strong and bullish and good_market and displacement_valid
        sell_strong = strong and bearish and good_market and displacement_valid

        # 🟡 MID
        buy_mid = mid and bullish and good_market and structure_ok
        sell_mid = mid and bearish and good_market and structure_ok

        # 🟠 WEAK
        buy_weak = weak and bias_strength > 0.40 and bos_up
        sell_weak = weak and bias_strength < -0.40 and bos_down
